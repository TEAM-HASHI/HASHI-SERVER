...
- 시작 시 publication 재전송, 정체 작업 복구, 지표 갱신은 각각 실패를 격리한다. 호출하는
  bean의 transaction 시작·종료 오류도 바깥에서 처리하고, 작업명과 오류 유형을 기록한 뒤
  기존 정기 실행에서 다시 시도한다. 필수 bean 생성·설정 검증 오류나 JVM Error는 숨기지 않는다.
  기본 실행 간격은 publication 1분, 정체 작업 15분, 지표 갱신 30초이며 이전 실행 종료 후
  기다리는 시간이다. 정체 작업의 첫 정기 실행은 시작 후 10분이며 대상 조건과 재시도 상한은 유지한다.
