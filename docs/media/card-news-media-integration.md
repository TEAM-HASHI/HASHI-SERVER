...
- 후보 박스가 원본보다 커도 확대하지 않는다. 출력 폭이 같으면 저장 주소도 같으므로,
  높이가 가장 큰 결과 하나만 생성한다. Spring과 worker에 같은 선택 기준을 적용한다.
- 기본 이미지는 nominal 864 후보 박스에서 계산한 실제 폭에 해당하는 최종 결과를 선택한다.
  높이는 원본 비율로 계산한 뒤 half-up으로 정수화하며, worker는 계산된 폭·높이로 출력한다.
