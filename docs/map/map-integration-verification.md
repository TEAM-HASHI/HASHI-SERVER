...
| --- | --- | --- |
| #220·#222·#224 | `0d77d91` | 시작 tree에 포함. 위치 모델·adapter·관리자 저장/작업 경로 |
| #223 | `98299f2`, `303fb5c` | `a592d1d`, `9f7e3e4`. 지도 오류 011~018과 위치 재처리 019, 두 Port 의존성을 함께 유지 |
| #227 | `31a7998` | `291b46a`. 지도 페이지와 Redis 세션. 선행 리뷰 수정이 있으면 추가 결합 필요 |
...
- #216 컬렉션이 준비되면 전체 저장 관계/권한/저장 수를 식당 Port와 연결하고, 권한 변동·부분 실패·비공개 404를 별도로 검증한다. 현재 첫 지도 페이지의 카드는 개인 저장 상태를 포함하지 않는다.
