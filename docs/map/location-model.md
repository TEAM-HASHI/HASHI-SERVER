...
없는 값을 거절하고, 뒤에 붙은 0은 허용한다. 외부 응답을 이 정밀도로 변환하는 정책은 adapter에서
명시해야 한다. MySQL DECIMAL은 직접 SQL의 초과 소수 자릿수를 반올림할 수 있으므로
...
이름은 1~100자, displayOrder는 0 이상이다. 지역은 명시적으로 활성화하기 전까지 비활성이다.
