CREATE TABLE restaurant_media_backfill_checkpoint (
    ...
    PRIMARY KEY (run_id),
    CONSTRAINT ck_restaurant_media_backfill_run_id CHECK (
        run_id REGEXP '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
    ),
    CONSTRAINT ck_restaurant_media_backfill_target CHECK (
        target IN ('RESTAURANT_IMAGE', 'RESTAURANT_MENU')
    ),
    CONSTRAINT ck_restaurant_media_backfill_mode CHECK (
        mode IN ('PREPARE', 'ATTACH')
    ),
    CONSTRAINT ck_restaurant_media_backfill_status CHECK (
        status IN ('PAUSED', 'RUNNING', 'COMPLETED')
    ),
    CONSTRAINT ck_restaurant_media_backfill_cursor CHECK (
        upper_bound_id >= 0 AND cursor_id >= 0 AND cursor_id <= upper_bound_id
    ),
    CONSTRAINT ck_restaurant_media_backfill_counts CHECK (
        scanned_count >= 0
    ...
    CONSTRAINT ck_restaurant_media_backfill_lease CHECK (
