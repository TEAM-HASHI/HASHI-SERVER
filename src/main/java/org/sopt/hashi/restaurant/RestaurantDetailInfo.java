...
/**
 * 모듈 간 전달용 식당 상세 DTO. 요약({@link RestaurantInfo}: id·name·address·imageUrl)보다 많은 표시 정보가
 * 필요한 조회(예약 상세, 매거진 연결 식당 카드)에 쓴다. 요약만 필요한 목록은 이 타입을 받지 않는다 —
 * 영업시간까지 함께 읽어 조회 비용이 더 들기 때문이다.
 * {@code imageUrl}은 대표 이미지, {@code imageUrls}는 식당 이미지 전체를 노출 순서대로 담은 조회 URL이다
 * (coding-style §4-2). 이미지가 없으면 {@code imageUrl}은 null, {@code imageUrls}는 빈 목록이다.
