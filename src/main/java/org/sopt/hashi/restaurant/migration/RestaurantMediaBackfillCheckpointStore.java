...
@Repository
class RestaurantMediaBackfillCheckpointStore {

    private static final String INSERT_IF_ABSENT_SQL = """
            INSERT INTO restaurant_media_backfill_checkpoint (
    ...
    RestaurantMediaBackfillCheckpointStore(JdbcTemplate jdbcTemplate) {
    ...
    public Acquisition acquire(
        ...
        if (updated != 1) {
            throw new RestaurantMediaBackfillLeaseLostException();
        }
    ...
    public void recordProgress(
            Lease lease,
            long nextCursor,
            RestaurantMediaBackfillOutcome outcome,
            Duration leaseDuration
    ) {
    ...
    private Snapshot mapSnapshot(ResultSet resultSet, int rowNumber) throws SQLException {
        ...
        return new Snapshot(
                ...
                RestaurantMediaBackfillTarget.valueOf(resultSet.getString("target")),
                RestaurantMediaBackfillMode.valueOf(resultSet.getString("mode")),
    ...
    private void requirePersistentMode(RestaurantMediaBackfillMode mode) {
    ...
    record Lease(
        ...
        Lease {
            ...
            if (!mode.usesCheckpoint() || upperBoundId < 0) {
                throw new IllegalArgumentException("invalid restaurant media backfill lease");
            }
        }

        @Override
        public String toString() {
            return "RestaurantMediaBackfillLease[redacted]";
        }
    }
    ...
    record Snapshot(
        ...
        @Override
        public String toString() {
            return "RestaurantMediaBackfillCheckpoint[redacted]";
        }
    }

    private record CounterDelta(long prepared, long attached, long skipped, long failed) {

        private static CounterDelta from(RestaurantMediaBackfillOutcome outcome) {
