...
class UserProfileBackfillDryRunFailureTest {
    ...
    void 다음_batch_조회_실패에도_이미_관측한_source_실패를_부분_결과에_보존한다() {
        ...
        given(reader.findBatch(1L, 2L, 1)).willThrow(new IllegalStateException(PAYLOAD));

        UserProfileBackfillSummary summary = runner().execute();
    ...
    void 종료_interrupt에도_부분_결과와_interrupt_flag를_보존한다() {
        ...
        try {
            ...
            verify(reader, never()).findBatch(1L, 2L, 1);
            verifyNoInteractions(checkpoint, attachment);
        } finally {
    ...
    void 최초_조회가_실패하면_실패한_실행과_비어_있는_관측을_보고한다() {
        given(reader.findUpperBound()).willThrow(new IllegalStateException(PAYLOAD));
    ...
    private void stubMissingFirstSource() {
        given(reader.findUpperBound()).willReturn(2L);
        given(reader.findBatch(0L, 2L, 1))
                .willReturn(List.of(new UserProfileBackfillCandidate(1L, KEY)));
        given(port.inspect(any())).willThrow(new MediaBackfillSourceException(Reason.SOURCE_MISSING));
    }

    private UserProfileBackfillRunner runner() {
        UserProfileBackfillProperties properties = new UserProfileBackfillProperties(
                true, "", UserProfileBackfillMode.DRY_RUN, 1, 3, Duration.ofMinutes(5), 3, Duration.ZERO);
        return new UserProfileBackfillRunner(properties, reader, checkpoint, attachment, port, registry);
    }

    private void assertPartialFailure(UserProfileBackfillSummary summary) {
        assertThat(summary.status()).isEqualTo(UserProfileBackfillSummary.Status.FAILED);
        ...
        assertThat(registry.get(UserProfileBackfillSourceFailures.METRIC_NAME)
