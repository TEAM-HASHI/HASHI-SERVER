...
class MagazineMediaBackfillDryRunFailureTest {
    ...
    void 다음_batch_조회_실패에도_이미_관측한_source_실패를_부분_결과에_보존한다() {
        ...
        MagazineMediaBackfillSummary summary = runner().execute();
    ...
    private void stubMissingFirstSource() {
        ...
        given(reader.findBatch(TARGET, 0L, 2L, 1))
                .willReturn(List.of(new MagazineMediaBackfillCandidate(TARGET, 1L, KEY)));
        given(port.inspect(any())).willThrow(new MediaBackfillSourceException(Reason.SOURCE_MISSING));
    }

    private MagazineMediaBackfillRunner runner() {
        MagazineMediaBackfillProperties properties = new MagazineMediaBackfillProperties(
                true, "", TARGET, MagazineMediaBackfillMode.DRY_RUN, 1, 3, Duration.ofMinutes(5), 3, Duration.ZERO);
        return new MagazineMediaBackfillRunner(properties, reader, checkpoint, attachment, port, registry);
    }

    private void assertPartialFailure(MagazineMediaBackfillSummary summary) {
        assertThat(summary.status()).isEqualTo(MagazineMediaBackfillSummary.Status.FAILED);
        ...
        assertThat(registry.get(MagazineMediaBackfillSourceFailures.METRIC_NAME)
