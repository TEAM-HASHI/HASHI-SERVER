...
class MagazineMediaBackfillUnreadableStopTest {
    private static final String IDENTITY = "a".repeat(64);

    static Stream<Arguments> cases() {
        return Arrays.stream(MagazineMediaBackfillTarget.values()).flatMap(target ->
                Arrays.stream(MagazineMediaBackfillMode.values()).flatMap(mode ->
                        mode == MagazineMediaBackfillMode.PREPARE
    ...
    void batch를_넘어_다섯번째_오류에서_멈추고_같은_실행을_재개한다(
            MagazineMediaBackfillTarget target, MagazineMediaBackfillMode mode, boolean copyFailure) {
        try (Harness h = new Harness(target, mode, copyFailure, "UUUUUU")) {
            MagazineMediaBackfillSummary stopped = h.runner.execute();
            assertThat(stopped.status()).isEqualTo(MagazineMediaBackfillSummary.Status.FAILED);
            ...
            MagazineMediaBackfillSummary resumed = h.runner.execute();
            assertThat(resumed.status()).isEqualTo(MagazineMediaBackfillSummary.Status.COMPLETED);
    ...
    void 네번의_오류는_중단하지_않고_다른_결과가_사이에_있으면_계수를_초기화한다(
        ...
        for (String pattern : List.of("UUUU", "UUUUSUUUU", "UUUUBUUUU", "UUUUMUUUU")) {
            try (Harness h = new Harness(target, mode, copyFailure, pattern)) {
                MagazineMediaBackfillSummary result = h.runner.execute();
                assertThat(result.status()).as(pattern).isEqualTo(MagazineMediaBackfillSummary.Status.COMPLETED);
    ...
    private static final class Harness implements AutoCloseable {
        ...
        private final MagazineMediaBackfillAttachmentService attachment = mock(MagazineMediaBackfillAttachmentService.class);
        ...
        private final MagazineMediaBackfillRunner runner;
        private final List<MagazineMediaBackfillCandidate> candidates = new ArrayList<>();
        private String pattern;

        private Harness(MagazineMediaBackfillTarget target, MagazineMediaBackfillMode mode, boolean copyFailure, String pattern) {
            this.target = target;
            this.mode = mode;
            ...
            runner = new MagazineMediaBackfillRunner(properties, reader, checkpoint, attachment, port, registry);
            for (int i = 1; i <= pattern.length(); i++) {
                String key = pattern.charAt(i - 1) == 'B' ? " " : "fixture/" + i + ".jpg";
                candidates.add(new MagazineMediaBackfillCandidate(target, i, key));
            }
            ...
            given(reader.findBatch(eq(target), anyLong(), anyLong(), anyInt()))
                    .willAnswer(call -> {
                        ...
                        return candidates.stream().filter(c -> c.magazineId() > from && c.magazineId() <= upper)
                                .limit(limit).toList();
                    });
            ...
            doAnswer(call -> {
                ...
                if (call.getArgument(2) == MagazineMediaBackfillOutcome.FAILED) {
