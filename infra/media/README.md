...
| `MEDIA_BACKFILL_ACCESS_ENABLED` | 승인된 legacy backfill용 추가 S3 권한. 생략 시 `false` |
...
1. 임시 backfill IAM 범위와 기본 비활성화 계약 테스트
2. worker exact dependency 설치와 fixture test
3. Linux x64 production package 생성
4. `sam validate --lint`
5. `sam build`
...
`BackfillAccessEnabled=false`가 기본값이다. 승인 후 GitHub Environment의
`MEDIA_BACKFILL_ACCESS_ENABLED=true`로 배포한 경우에만 별도
